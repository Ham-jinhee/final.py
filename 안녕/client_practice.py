...
form_class = uic.loadUiType("multi_chatting.ui")[0]
...
class MyWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        self.user_pw = []
        self.user_ids = []
        self.client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # 소켓 만들기
        self.setupUi(self)
        self.Host = '10.10.20.106' # host ip
        self.Port = 8002 # port 번호
        self.client_socket.connect((self.Host, self.Port))  # 클라이언트 소켓에 호스트와 포트번호 연결
        self.check_cnt = 0
        self.user_id = ''
        self.room = ''
        self.stackedWidget.setCurrentWidget(self.login_page)
        self.login_btn.clicked.connect(self.login) # 로그인
        self.signup_btn.clicked.connect(self.signup) # 회원가입
        self.make_room_btn.clicked.connect(self.make_room) # 방만들기 버튼
        self.refresh_btn.clicked.connect(self.refresh) # 새로고침
        self.room_list.itemDoubleClicked.connect(self.room_join) # 더블클릭해서 채팅방 들어가기
        self.room_send_btn.clicked.connect(self.write) # 메세지 전송
        self.room_exit_btn.clicked.connect(self.exit_room) # 방 나가기
